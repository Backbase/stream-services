...
public abstract class UnitOfWorkExecutor<T extends StreamTask> {
    ...
    public UnitOfWorkExecutor(
        ...
        this.taskExecutor =
            Schedulers.newParallel("TaskScheduler", streamWorkerConfiguration.getTaskExecutors());
        ...
        if (failed) {
            ...
            if (retries < streamWorkerConfiguration.getMaxRetries()) {
                unitOfWork.setNextAttemptAt(
                    OffsetDateTime.now()
                        .plusSeconds(streamWorkerConfiguration.getRetryDuration().getSeconds()));
    ...
    private T endTask(UnitOfWork<T> unitOfWork, T streamTask) {
        log.info(
            "Ending Task: {} from Unit Of Work: {}", streamTask.getId(), unitOfWork.getUnitOfOWorkId());
