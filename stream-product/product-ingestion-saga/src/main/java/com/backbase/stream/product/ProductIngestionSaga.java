...
public class ProductIngestionSaga {
  ...
  private Mono<ProductGroupTask> validateProductGroup(ProductGroupTask streamTask) {
    ...
    if (productGroup.getProductGroupType() == null
        && StreamUtils.getAllProducts(productGroup).findAny().isPresent()) {
      streamTask.warn(
          PRODUCT_GROUP,
          VALIDATE,
          REJECTED,
          name,
          null,
          "Product Group: {} does not have type setup. As Product Group contains products setting type to ARRANGEMENTS");
        ...
        .zipWith(
            ...
            (actual, arrangements) -> {
              List<CurrentAccount> collect =
                  arrangements.stream().map(productMapper::mapCurrentAccount).toList();
        ...
        .zipWith(
            ...
            (actual, arrangements) ->
                actual.savingAccounts(
                    arrangements.stream().map(productMapper::mapSavingAccount).toList()))
        .zipWith(
            debitCardsRequests,
            (actual, arrangements) ->
                actual.debitCards(arrangements.stream().map(productMapper::mapDebitCard).toList()))
        .zipWith(
            creditCardsRequests,
            (actual, arrangements) ->
                actual.creditCards(
                    arrangements.stream().map(productMapper::mapCreditCard).toList()))
        .zipWith(
            loansRequests,
            (actual, arrangements) ->
                actual.loans(arrangements.stream().map(productMapper::mapLoan).toList()))
        .zipWith(
            termDepositsRequests,
            (actual, arrangements) ->
                actual.termDeposits(
                    arrangements.stream().map(productMapper::mapTermDeposit).toList()))
        .zipWith(
            investmentAccountsRequests,
            (actual, arrangements) ->
                actual.investmentAccounts(
                    arrangements.stream().map(productMapper::mapInvestmentAccount).toList()))
        .zipWith(
            customProductsRequests,
            (actual, arrangements) ->
                actual.customProducts(
                    arrangements.stream().map(productMapper::mapCustomProduct).toList()))
        .cast(ProductGroup.class)
        .map(streamTask::data);
  }

  private Mono<List<ArrangementItem>> upsertArrangements(
      ProductGroupTask streamTask, Flux<PostArrangement> productFlux) {
    ProductGroup productGroup = streamTask.getData();
    return productFlux
        .map(p -> ensureLegalEntityId(productGroup.getUsers(), p))
        .sort(
            comparing(
                PostArrangement::getParentId,
                nullsFirst(naturalOrder()))) // Avoiding child to be created before parent
        .flatMapSequential(
  ...
  /**
   * Create a postArrangement in DBS. If postArrangement already exists, return existing
   * postArrangement
   *
   * @param postArrangement Product to create
   * @return Created or Existing Product
   */
  public Mono<ArrangementItem> upsertArrangement(
      ProductGroupTask streamTask, PostArrangement postArrangement) {
    streamTask.info(
        ARRANGEMENT,
        UPSERT_ARRANGEMENT,
        "",
    ...
    Mono<ArrangementItem> updateArrangement =
        arrangementService
            .getArrangementInternalId(postArrangement.getId())
            .flatMap(
                internalId -> {
                  ...
                  streamTask.info(
                      ...
                      postArrangement.getId());
                  ArrangementPutItem arrangemenItemBase =
                      productMapper.toArrangementItemPut(postArrangement);
                  return arrangementService
                      .updateArrangement(internalId, arrangemenItemBase)
                      .onErrorResume(
    ...
    Mono<ArrangementItem> createNewArrangement =
        createArrangement(postArrangement)
            .map(
                arrangementItem -> {
                  streamTask.info(
                      ARRANGEMENT,
  ...
  protected PostArrangement ensureLegalEntityId(
      List<JobProfileUser> users, PostArrangement product) {
