...
public class ProductIngestionSaga {
  ...
  protected final BusinessFunctionGroupMapper businessFunctionGroupMapper =
        ...
        .doOnNext(
            productGroupTask -> {
              streamTask.info(
                  ...
                  null,
                  "Process Product Group Task: %s",
                  productGroupTask.getId());
            })
  ...
  private Mono<? extends ProductGroupTask> setupBusinessFunctionsAndPermissions(
      ProductGroupTask streamTask) {
    ProductGroup productGroup = streamTask.getData();
    return Flux.fromIterable(productGroup.getUsers())
        .flatMap(
            jobProfileUser ->
                setupBusinessFunctions(
                    streamTask, streamTask.getData().getServiceAgreement(), jobProfileUser))
  ...
  private Mono<ProductGroupTask> validateProductGroup(ProductGroupTask streamTask) {
    ...
    if (productGroup.getUsers() == null) {
      streamTask.warn(
          ...
          name,
          null,
          "Product Group must have users assigned to it!");
      return Mono.error(
          new StreamTaskException(streamTask, "Product Group must have users assigned to it!"));
    }
    if (StreamUtils.getInternalProductIds(productGroup).isEmpty()
        && customDataGroupItems.isEmpty()) {
      streamTask.warn(
          PRODUCT_GROUP,
          VALIDATE,
          REJECTED,
          name,
          null,
          "Product group must have products or Custom Data Group Items assigned to it!");
      return Mono.error(
          new StreamTaskException(
              streamTask,
              "Product group must have products or Custom Data Group Items assigned" + " to it!"));
    }
    if (customDataGroupItems != null
        && !customDataGroupItems.isEmpty()
        && productGroup.getProductGroupType() == null) {
      streamTask.warn(
          PRODUCT_GROUP,
          VALIDATE,
          REJECTED,
          name,
          null,
          "Product Group with Custom Data Group Items must have a Product Group" + " Defined!");
      return Mono.error(
          new StreamTaskException(
              streamTask,
              "Product Group with Custom Data Group Items must have a Product Group"
                  + " Defined!"));
    }
    if (productGroup.getProductGroupType() == null
        && StreamUtils.getAllProducts(productGroup).count() > 0) {
      streamTask.warn(
          PRODUCT_GROUP,
          VALIDATE,
          REJECTED,
          name,
          null,
          "Product Group: {} does not have type setup. As Product Group contains products"
              + " setting type to ARRANGEMENTS");
  ...
  private Mono<JobProfileUser> setupBusinessFunctions(
      StreamTask streamTask, ServiceAgreement serviceAgreement, JobProfileUser jobProfileUser) {
    streamTask.info(
        FUNCTION_GROUP,
        ...
        "",
        null,
        "Setting up Business Functions User: %s",
  ...
  protected Mono<List<BusinessFunctionGroup>> getBusinessFunctionGroups(
      JobProfileUser jobProfileUser, ServiceAgreement serviceAgreement) {
    ...
    if (!isEmpty(jobProfileUser.getReferenceJobRoleNames())) {
      return accessGroupService
          .getFunctionGroupsForServiceAgreement(serviceAgreement.getInternalId())
          .map(
              functionGroups -> {
  ...
  //
  //                Map<User, Map<BusinessFunctionGroup, List<BaseProductGroup>>> request = new
  // HashMap<>();
  ...
  //                        .serviceAgreement(streamTask.getProductGroup().getServiceAgreement()),
  // BatchProductGroupTask.IngestionMode.UPDATE), request)
  //                    .thenReturn(jobProfileUser);
  //            });
  //    }

  private Mono<JobProfileUser> setupPermissions(
  ...
  protected Mono<User> processUser(StreamTask streamTask, JobProfileUser jobProfileUser) {
    if (configurationProperties.isIdentityEnabled()
        && !IdentityUserLinkStrategy.IDENTITY_AGNOSTIC.equals(
            jobProfileUser.getUser().getIdentityLinkStrategy())) {
  ...
  private Mono<User> upsertUser(StreamTask streamTask, JobProfileUser jobProfileUser) {
    ...
    Mono<User> createNewUser =
        userService
            .createUser(user, legalEntityReference.getExternalId(), streamTask)
            .doOnNext(
                existingUser ->
                    streamTask.info(
                        USER,
  ...
  private Mono<User> upsertIdentityUser(StreamTask streamTask, JobProfileUser jobProfileUser) {
    ...
    Mono<User> createNewIdentityUser =
        userService
            .createOrImportIdentityUser(user, legalEntityReference.getInternalId(), streamTask)
            .doOnNext(
                existingUser ->
                    streamTask.info(
                        IDENTITY_USER,
  ...
  public Mono<ProductGroupTask> upsertArrangements(ProductGroupTask streamTask) {
    ...
    streamTask.info(
        ...
        streamTask.getProductGroup().getName(),
        null,
        "Process Product Group Arrangements: %s",
        StreamUtils.getExternalProductIds(productGroup));
    ...
    Mono<List<AccountArrangementItem>> currentAccountsRequests =
        upsertArrangements(streamTask, currentAccountFlux.map(productMapper::toPresentation));
    Mono<List<AccountArrangementItem>> savingAccountsRequests =
        upsertArrangements(streamTask, savingsAccountFlux.map(productMapper::toPresentation));
    Mono<List<AccountArrangementItem>> debitCardsRequests =
        upsertArrangements(streamTask, debitCardFlux.map(productMapper::toPresentation));
    Mono<List<AccountArrangementItem>> creditCardsRequests =
        upsertArrangements(streamTask, creditCardFlux.map(productMapper::toPresentation));
    Mono<List<AccountArrangementItem>> loansRequests =
        upsertArrangements(streamTask, loanFlux.map(productMapper::toPresentation));
    Mono<List<AccountArrangementItem>> termDepositsRequests =
        upsertArrangements(streamTask, termDepositFlux.map(productMapper::toPresentation));
    Mono<List<AccountArrangementItem>> investmentAccountsRequests =
        upsertArrangements(streamTask, investmentAccountFlux.map(productMapper::toPresentation));
    Mono<List<AccountArrangementItem>> customProductsRequests =
        upsertArrangements(streamTask, productFlux.map(productMapper::toPresentation));

    return Mono.just(productGroup)
        .zipWith(
            currentAccountsRequests,
            (actual, arrangements) -> {
              List<CurrentAccount> collect =
                  arrangements.stream()
                      .map(productMapper::mapCurrentAccount)
                      .collect(Collectors.toList());
              return actual.currentAccounts(collect);
            })
        .zipWith(
            savingAccountsRequests,
            (actual, arrangements) ->
                actual.savingAccounts(
                    arrangements.stream()
                        .map(productMapper::mapSavingAccount)
                        .collect(Collectors.toList())))
        .zipWith(
            debitCardsRequests,
            (actual, arrangements) ->
                actual.debitCards(
                    arrangements.stream()
                        .map(productMapper::mapDebitCard)
                        .collect(Collectors.toList())))
        .zipWith(
            creditCardsRequests,
            (actual, arrangements) ->
                actual.creditCards(
                    arrangements.stream()
                        .map(productMapper::mapCreditCard)
                        .collect(Collectors.toList())))
        .zipWith(
            loansRequests,
            (actual, arrangements) ->
                actual.loans(
                    arrangements.stream().map(productMapper::mapLoan).collect(Collectors.toList())))
        .zipWith(
            termDepositsRequests,
            (actual, arrangements) ->
                actual.termDeposits(
                    arrangements.stream()
                        .map(productMapper::mapTermDeposit)
                        .collect(Collectors.toList())))
        .zipWith(
            investmentAccountsRequests,
            (actual, arrangements) ->
                actual.investmentAccounts(
                    arrangements.stream()
                        .map(productMapper::mapInvestmentAccount)
                        .collect(Collectors.toList())))
        .zipWith(
            customProductsRequests,
            (actual, arrangements) ->
                actual.customProducts(
                    arrangements.stream()
                        .map(productMapper::mapCustomProduct)
                        .collect(Collectors.toList())))
        .cast(ProductGroup.class)
        .map(streamTask::data);
  }

  private Mono<List<AccountArrangementItem>> upsertArrangements(
      ProductGroupTask streamTask, Flux<AccountArrangementItemPost> productFlux) {
    ProductGroup productGroup = streamTask.getData();
    return productFlux
        .map(p -> ensureLegalEntityId(productGroup.getUsers(), p))
        .sort(
            comparing(
                AccountArrangementItemPost::getExternalParentId,
                nullsFirst(naturalOrder()))) // Avoiding child to be created before
        // parent
        .flatMapSequential(
            arrangementItemPost -> upsertArrangement(streamTask, arrangementItemPost))
        .collectList();
  }

  /**
   * Create a arrangementItemPost in DBS. If arrangementItemPost already exists, return existing
   * arrangementItemPost
   *
   * @param arrangementItemPost Product to create
   * @return Created or Existing Product
   */
  public Mono<AccountArrangementItem> upsertArrangement(
      ProductGroupTask streamTask, AccountArrangementItemPost arrangementItemPost) {
    streamTask.info(
        ARRANGEMENT,
        UPSERT_ARRANGEMENT,
        "",
        ...
        null,
        "Inserting or updating arrangement: %s",
        arrangementItemPost.getExternalArrangementId());
    log.info(
        "Upsert Arrangement: {} in Product Group: {}",
        arrangementItemPost.getExternalArrangementId(),
        streamTask.getData().getName());
    Mono<AccountArrangementItem> updateArrangement =
        arrangementService
            .getArrangementInternalId(arrangementItemPost.getExternalArrangementId())
            .flatMap(
                internalId -> {
                  ...
                  streamTask.info(
                      ...
                      arrangementItemPost.getExternalArrangementId(),
                      internalId,
                      "Arrangement %s already exists",
                      arrangementItemPost.getExternalArrangementId());
                  AccountArrangementItemPut arrangemenItemBase =
                      productMapper.toArrangementItemPut(arrangementItemPost);
                  return arrangementService
                      .updateArrangement(arrangemenItemBase)
                      .onErrorResume(
                          ArrangementUpdateException.class,
                          e -> {
                            streamTask.error(
                                ARRANGEMENT,
                                UPDATE_ARRANGEMENT,
                                ...
                                e,
                                e.getHttpResponse(),
                                "Failed to update arrangement: %s",
                                arrangementItemPost.getExternalArrangementId());
                            return Mono.error(
                                new StreamTaskException(
                                    streamTask, e, "Failed to update" + " arrangement"));
                          })
                      .map(
                          actual -> {
                            ...
                            AccountArrangementItem arrangementItem =
                                productMapper.toArrangementItem(actual);
                            arrangementItem.setId(internalId);
                            return arrangementItem;
                          });
                });
    Mono<AccountArrangementItem> createNewArrangement =
        createArrangement(arrangementItemPost)
            .map(
                arrangementItem -> {
                  streamTask.info(
                      ARRANGEMENT,
                      ...
                      "created",
                      arrangementItemPost.getExternalArrangementId(),
                      arrangementItem.getId(),
                      "Created Arrangement");
                  log.info(
                      "Created arrangement: {} with internalId: {} ",
                      arrangementItem.getExternalArrangementId(),
                      arrangementItem.getId());
                  return arrangementItem;
                })
            .onErrorResume(
                ArrangementCreationException.class,
                e -> {
                  streamTask.error(
                      "ARRANGEMENT",
                      "insert-arrangement",
                      ...
                      arrangementItemPost.getExternalArrangementId());
                  return Mono.error(
                      new StreamTaskException(streamTask, e, "Failed to create arrangement"));
                });

    return updateArrangement.switchIfEmpty(createNewArrangement);
  }

  private Mono<AccountArrangementItem> createArrangement(
  ...
  protected AccountArrangementItemPost ensureLegalEntityId(
      List<JobProfileUser> users, AccountArrangementItemPost product) {
    Set<String> legalEntityExternalIds =
        users.stream()
            .map(jobProfileUser -> jobProfileUser.getLegalEntityReference().getExternalId())
            .collect(Collectors.toSet());
    // Make sure that we take into consideration Legal Entity Ids provided in Arrangement Item
    // itself.
