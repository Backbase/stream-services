...
public class ProductIngestionSaga {
                ...
                .flatMap(
                        jobProfileUser ->
                                setupBusinessFunctions(
                                        streamTask,
                                        streamTask.getData().getServiceAgreement(),
                                        jobProfileUser))
    ...
    private Mono<ProductGroupTask> validateProductGroup(ProductGroupTask streamTask) {
        ...
        if (productGroup.getUsers() == null) {
            ...
            return Mono.error(
                    new StreamTaskException(
                            streamTask, "Product Group must have users assigned to it!"));
        }
        if (StreamUtils.getInternalProductIds(productGroup).isEmpty()
            ...
            return Mono.error(
                    new StreamTaskException(
                            streamTask,
                            "Product group must have products or Custom Data Group Items assigned"
                                    + " to it!"));
        }
    ...
    private Mono<JobProfileUser> setupPermissions(
        ...
        return processUser(streamTask, jobProfileUser)
                ...
                .flatMap(
                        actualUser ->
                                accessGroupService.assignPermissions(streamTask, jobProfileUser));
    }
        ...
        Mono<User> createNewIdentityUser =
                userService
                        .createOrImportIdentityUser(
                                user, legalEntityReference.getInternalId(), streamTask)
                        .doOnNext(
                                existingUser ->
                                        streamTask.info(
                                                IDENTITY_USER,
    ...
    public Mono<ProductGroupTask> upsertArrangements(ProductGroupTask streamTask) {
        ...
        Mono<List<AccountArrangementItem>> currentAccountsRequests =
                upsertArrangements(
                        streamTask, currentAccountFlux.map(productMapper::toPresentation));
        Mono<List<AccountArrangementItem>> savingAccountsRequests =
                upsertArrangements(
                        streamTask, savingsAccountFlux.map(productMapper::toPresentation));
        ...
        Mono<List<AccountArrangementItem>> investmentAccountsRequests =
                upsertArrangements(
                        streamTask, investmentAccountFlux.map(productMapper::toPresentation));
                                    ...
                                    return arrangementService
                                            ...
                                            .map(
                                                    actual -> {
                                                        log.info(
                                                                "Updated arrangement: {}",
                                                                actual.getExternalArrangementId());
                                                        streamTask.info(
                                                                ARRANGEMENT,
                                                                UPSERT_ARRANGEMENT,
                                                                UPDATED,
                                                        ...
                                                        AccountArrangementItem arrangementItem =
                                                                productMapper.toArrangementItem(
                                                                        actual);
                ...
                createArrangement(arrangementItemPost)
                        ...
                        .onErrorResume(
                                ...
                                e -> {
                                    ...
                                    return Mono.error(
                                            new StreamTaskException(
                                                    streamTask, e, "Failed to create arrangement"));
                                });
