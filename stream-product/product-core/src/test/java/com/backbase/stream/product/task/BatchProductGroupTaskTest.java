...
public class BatchProductGroupTaskTest {
    ...
    private final BatchProductGroup batchProductGroup = new BatchProductGroup()
    ...
    void create_BatchProductGroup() {
        BatchProductGroupTask batchProductGroupTask = new BatchProductGroupTask("bpg_task_id", batchProductGroup);
        Assertions.assertEquals("bpg_task_id", batchProductGroupTask.getName());
        Assertions.assertEquals(BatchProductIngestionMode.UPSERT, batchProductGroupTask.getIngestionMode());
        Assertions.assertEquals("sa_int_id", batchProductGroupTask.getData().getServiceAgreement().getInternalId());
        Assertions.assertEquals(2, batchProductGroupTask.getData().getProductGroups().size());
    }

    @Test
    void create_BatchProductGroup_IngestionMode() {
        BatchProductGroupTask batchProductGroupTask = new BatchProductGroupTask("bpg_task_id", batchProductGroup, BatchProductIngestionMode.REPLACE);
        Assertions.assertEquals("bpg_task_id", batchProductGroupTask.getName());
        Assertions.assertEquals(BatchProductIngestionMode.REPLACE, batchProductGroupTask.getIngestionMode());
        Assertions.assertEquals("sa_int_id", batchProductGroupTask.getData().getServiceAgreement().getInternalId());
        Assertions.assertEquals(2, batchProductGroupTask.getData().getProductGroups().size());
    }
