...
public class PaymentOrderTaskTest extends PaymentOrderBaseTest {
    ...
    private final List<PaymentOrderIngestRequest> paymentOrderIngestRequestList = List.of(
    ...
    void create_PaymentOrderIngestRequest() {
        ...
        Assertions.assertEquals(paymentOrderIngestRequestList.size(), paymentOrderTask.getData().size());
        for(int idx=0; idx < paymentOrderTask.getData().size(); idx++) {
            Assertions.assertEquals(paymentOrderIngestRequestList.get(idx).getBankReferenceId(), paymentOrderTask.getData().get(idx).getBankReferenceId());
        }
    }
