...
public class TransactionTaskExecutor implements StreamTaskExecutor<TransactionTask> {
  ...
  public TransactionTaskExecutor(
  ...
  public Mono<TransactionTask> executeTask(TransactionTask streamTask) {
    ...
    String externalIds =
        streamTask.getData().stream()
            .map(TransactionsPostRequestBody::getExternalId)
            .collect(Collectors.joining(","));
    log.info("Post {} transactions: ", data.size());
    return transactionPresentationServiceApi
        .postTransactions(data, null, null, null)
        .onErrorResume(
