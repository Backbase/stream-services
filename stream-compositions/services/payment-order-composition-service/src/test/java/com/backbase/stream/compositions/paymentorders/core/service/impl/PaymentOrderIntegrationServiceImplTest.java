...
class PaymentOrderIntegrationServiceImplTest {
    ...
    @BeforeEach
    void setUp() {
        paymentOrderIntegrationService = new PaymentOrderIntegrationServiceImpl(
            paymentOrderIntegrationApi,
            paymentOrderMapper
        );
    }
    ...
    void pullPaymentOrderTest() {
        ...
        PullPaymentOrderResponse pullPaymentOrderResponse = new PullPaymentOrderResponse()
            .addPaymentOrderItem(paymentOrderPostRequest);

        when(paymentOrderIntegrationApi.pullPaymentOrders(pullIngestionRequest)).thenReturn(
            Mono.just(pullPaymentOrderResponse)
        );

        StepVerifier.create(paymentOrderIntegrationService.pullPaymentOrder(paymentOrderIngestPullRequest))
