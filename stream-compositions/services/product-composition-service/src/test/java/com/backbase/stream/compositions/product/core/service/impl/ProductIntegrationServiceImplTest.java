...
class ProductIntegrationServiceImplTest {
    ...
    void callIntegrationService_Success() throws UnsupportedOperationException {
        ...
        StepVerifier.create(productIntegrationService.pullProductGroup(request))
                .expectNextMatches(
                        response ->
                                response.getLegalEntityInternalId()
                                                .equals(expectedResponse.getLegalEntityInternalId())
                                        && response.getLegalEntityExternalId()
                                                .equals(expectedResponse.getLegalEntityExternalId())
                                        && response.getServiceAgreementInternalId()
                                                .equals(
                                                        expectedResponse
                                                                .getServiceAgreementInternalId())
                                        && response.getServiceAgreementExternalId()
                                                .equals(
                                                        expectedResponse
                                                                .getServiceAgreementExternalId())
                                        && response.getUserInternalId()
                                                .equals(expectedResponse.getUserInternalId())
                                        && response.getUserExternalId()
                                                .equals(expectedResponse.getUserExternalId())
                                        && response.getSource()
                                                .equals(expectedResponse.getSource()))
                .verifyComplete();
    }
