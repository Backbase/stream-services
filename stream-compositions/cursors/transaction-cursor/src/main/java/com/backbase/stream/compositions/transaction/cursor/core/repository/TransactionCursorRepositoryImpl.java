...
public class TransactionCursorRepositoryImpl implements TransactionCursorCustomRepository {
    ...
    public Integer patchByArrangementId(
        ...
        if (null != transactionCursorPatchRequest.getLastTxnDate()) {
            ...
            try {
                ...
                log.error(
                    "Parsing Exception during converting to timestamp {} ", parseException.getMessage());
                throw new RuntimeException(parseException);
            }
            cq.set(transactionCursor.get(LAST_TXN_DATE), lastTxnDate);
        }

        cq.where(equalPredicate(criteriaBuilder, transactionCursor.get(ARRANGEMENT_ID), arrangementId));
    ...
    public List<TransactionCursorEntity> filterCursor(
        ...
        try {
            txnDate =
                new SimpleDateFormat(DATE_FORMAT).parse(transactionCursorFilterRequest.getLastTxnDate());
        } catch (ParseException parseException) {
            log.error(
                "Parsing Exception during extracting transaction date {} ", parseException.getMessage());
            throw new RuntimeException(parseException);
        }
