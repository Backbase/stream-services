...
public class TransactionCursorRepositoryImpl implements TransactionCursorCustomRepository {
     ...
     */
    @Override
    public Integer patchByArrangementId(
            String arrangementId, TransactionCursorPatchRequest transactionCursorPatchRequest) {
        CriteriaBuilder criteriaBuilder = this.entityManager.getCriteriaBuilder();
        CriteriaUpdate<TransactionCursorEntity> cq =
                criteriaBuilder.createCriteriaUpdate(TransactionCursorEntity.class);
        ...
        if (null != transactionCursorPatchRequest.getLastTxnIds()) {
            cq.set(
                    transactionCursor.get(LAST_TXN_IDS),
                    transactionCursorPatchRequest.getLastTxnIds());
        }
        if (null != transactionCursorPatchRequest.getLastTxnDate()) {
            ...
            try {
                ...
                lastTxnDate =
                        new Timestamp(
                                simpleDateFormat
                                        .parse(transactionCursorPatchRequest.getLastTxnDate())
                                        .getTime());
            } catch (ParseException parseException) {
                log.error(
                        "Parsing Exception during converting to timestamp {} ",
    ...
    public List<TransactionCursorEntity> filterCursor(
        ...
        CriteriaBuilder criteriaBuilder = this.entityManager.getCriteriaBuilder();
        CriteriaQuery<TransactionCursorEntity> cq =
                criteriaBuilder.createQuery(TransactionCursorEntity.class);
        Root<TransactionCursorEntity> transactionCursor = cq.from(TransactionCursorEntity.class);
        Predicate statusPredicate =
                equalPredicate(
                        criteriaBuilder,
                        transactionCursor.get(STATUS),
                        transactionCursorFilterRequest.getStatus());
        ParameterExpression<java.util.Date> parameter =
                criteriaBuilder.parameter(java.util.Date.class);
        java.util.Date txnDate;
        try {
            txnDate =
                    new SimpleDateFormat(DATE_FORMAT)
                            .parse(transactionCursorFilterRequest.getLastTxnDate());
        } catch (ParseException parseException) {
            log.error(
                    "Parsing Exception during extracting transaction date {} ",
                    parseException.getMessage());
            throw new RuntimeException(parseException);
        }
        Predicate lastTxnDatePredicate =
                criteriaBuilder.lessThanOrEqualTo(
                        transactionCursor.get(LAST_TXN_DATE).as(java.sql.Date.class), parameter);

        Predicate filterPredicate = criteriaBuilder.and(statusPredicate, lastTxnDatePredicate);
        cq.where(filterPredicate);
        return this.entityManager.createQuery(cq).setParameter(parameter, txnDate).getResultList();
    }
    ...
    private Predicate equalPredicate(
            CriteriaBuilder criteriaBuilder, Path<String> expression, String value) {
