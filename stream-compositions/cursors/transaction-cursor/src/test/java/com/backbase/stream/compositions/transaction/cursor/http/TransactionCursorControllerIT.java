...
@WebFluxTest(
        controllers = TransactionCursorController.class,
        excludeAutoConfiguration = {TransactionCursorConfiguration.class})
public class TransactionCursorControllerIT {

    @Autowired TransactionCursorController transactionCursorController;

    @MockBean TransactionCursorRepository transactionCursorRepository;

    @MockBean TransactionCursorRepositoryImpl transactionCursorRepositoryImpl;

    @MockBean WebClient webClient;

    @Autowired private WebTestClient webTestClient;

    @Autowired TransactionCursorMapper mapper;
    ...
    void getById_Success() {
        ...
        webTestClient
                .get()
                .uri("/service-api/v2/cursor/{id}", id)
                .exchange()
                .expectStatus()
                .isOk()
                .expectBody()
                .jsonPath("$.cursor.id")
                .isNotEmpty();
        Mockito.verify(transactionCursorRepository, Mockito.times(1)).findById(id);
    }
    ...
    void filterCursor_Success() {
        ...
        webTestClient
                .post()
                .uri("/service-api/v2/cursor/filter")
                .contentType(MediaType.APPLICATION_JSON)
                .body(
                        Mono.just(transactionCursorFilterRequest),
                        TransactionCursorFilterRequest.class)
                .exchange()
                .expectStatus()
                .isOk();
    }
    ...
    void upsertCursor_Success() {
                ...
                new TransactionCursorUpsertRequest()
                        .withCursor(
                                new TransactionCursor()
                                        ...
                                        .withExtArrangementId(
                                                "5337f8cc-d66d-41b3-a00e-f71ff15d93cq")
        ...
        webTestClient
                .post()
                .uri("/service-api/v2/cursor/upsert")
                .contentType(MediaType.APPLICATION_JSON)
                .body(
                        Mono.just(transactionCursorUpsertRequest),
                        TransactionCursorUpsertRequest.class)
                .exchange()
                .expectStatus()
                .isOk();
        ...
        StepVerifier.create(transactionCursorRepository.save(any()))
                ...
                .verifyComplete();
        // Mockito.verify(transactionCursorRepository,
        // Mockito.times(1)).save(transactionCursorEntity);
    }
}
