...
public class UpdatedServiceAgreementSaga
    ...
    private final ProductGroupMapper productGroupMapper =
            Mappers.getMapper(ProductGroupMapper.class);
                ...
                () -> {
                    ...
                    return new StreamTaskException(
                            ...
                            MessageFormat.format(
                                    "Service Agreement with external id: {0}, was not found",
                                    sa.getExternalId()));
                });
    }
        ...
        Map<BaseProductGroup, ProductGroup> productGroupByBasePg =
                unmodifiableMap(
                        sa.getProductGroups().stream()
                                .collect(
                                        Collectors.toMap(
                                                Function.identity(), productGroupMapper::map)));
                        ...
                        list -> {
                            ...
                            if (list.stream().anyMatch(StreamTask::isFailed)) {
                                ...
                                task.error(
                                        ...
                                        sa.getInternalId(),
                                        sa.getExternalId(),
                                        "failed to setup Product Groups: %s for Service Agreement:"
                                                + " %s",
                                        ids,
                                        sa.getExternalId());
                                ...
                                return Mono.error(
                                        new StreamTaskException(
                                                task, "error on product group update"));
                            } else {
                                        ...
                                        .onErrorResume(
                                                ...
                                                e -> {
                                                    ...
                                                    task.error(
                                                            ...
                                                            e,
                                                            e.getMessage(),
                                                            "failure retrieving user for external"
                                                                    + " id: %s",
                                    ...
                                    .forEach(
                                            jp -> {
                                                ...
                                                GetUser internalUser =
                                                        usersByExternalId.get(externalId);
                                                if (internalUser != null) {
                                                    jp.getUser()
                                                            .setInternalId(internalUser.getId());
                                                } else {
                                                    task.error(
                                                            ENTITY_SERVICE_AGREEMENT,
                                                            OP_PROCESS_JOB_PROFILES,
        ...
        Map<String, BaseProduct> productByExternalId =
                unmodifiableMap(
                        sa.getProductGroups().stream()
                                ...
                                .collect(
                                        Collectors.toMap(
                                                BaseProduct::getExternalId, Function.identity())));
        return arrangementService
                ...
                .doOnNext(
                        arrangement -> {
                            ...
                            BaseProduct product =
                                    productByExternalId.get(arrangement.getExternalArrangementId());
                        ...
                        list -> {
                            productByExternalId.values().stream()
                                    ...
                                    .forEach(
                                            bp -> {
                                                log.error(
                                                        "Product with external id: {} not found",
                                                        bp.getExternalId());
                                                task.error(
                                                        ENTITY_SERVICE_AGREEMENT,
                                                        OP_PROCESS_JOB_PROFILES,
                            ...
                            if (task.isFailed()) {
                                return Mono.error(
                                        new StreamTaskException(
                                                task, "error retrieving product internal id"));
                            }
                            return Mono.just(task);
                        });
    }
                        ...
                        e -> {
                            ...
                            return Mono.error(
                                    new StreamTaskException(
                                            task,
                                            MessageFormat.format(
                                                    "Failed to retrieve Function Groups for Service"
                                                            + " Agreement: {0}",
                                                    sa.getExternalId())));
                        })
                .map(
    ...
    private Mono<UpdatedServiceAgreementTask> setupUserPermissions(
        ...
        return accessGroupService
                ...
                .onErrorResume(
                        e ->
                                e instanceof StreamTaskException
