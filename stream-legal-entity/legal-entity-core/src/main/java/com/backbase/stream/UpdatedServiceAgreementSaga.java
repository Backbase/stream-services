...
public class UpdatedServiceAgreementSaga
    implements StreamTaskExecutor<UpdatedServiceAgreementTask> {
    ...
    private Mono<UpdatedServiceAgreementTask> updateServiceAgreement(
        UpdatedServiceAgreementTask streamTask) {
        ...
        return accessGroupService
            ...
            .flatMap(
                existingSa -> {
                    ...
                    return accessGroupService.updateServiceAgreementAssociations(
                        streamTask, streamTask.getData(), serviceAgreement.getSaUsers());
                })
            .flatMap(
                sa -> {
                    streamTask.info(
                        ENTITY_SERVICE_AGREEMENT,
                        OP_UPDATE_SERVICE_AGREEMENT,
                        RESULT_UPDATED,
                        sa.getExternalId(),
                        sa.getInternalId(),
                        "Updated Service Agreement: %s",
                        sa.getExternalId());
                    return Mono.just(streamTask);
                })
    ...
    private Mono<UpdatedServiceAgreementTask> serviceAgreementDoesntExistError(
        UpdatedServiceAgreementTask streamTask) {
        ...
        return Mono.error(
            () -> {
                streamTask.error(
                    ...
                    sa.getExternalId(),
                    null,
                    "Service Agreement with external id: %s, was not found",
                    sa.getExternalId());
                return new StreamTaskException(
                    streamTask,
                    MessageFormat.format(
                        "Service Agreement with external id: {0}, was not found", sa.getExternalId()));
            });
    }
    ...
    private Mono<UpdatedServiceAgreementTask> processProducts(UpdatedServiceAgreementTask task) {
        ...
        Map<BaseProductGroup, ProductGroup> productGroupByBasePg =
            unmodifiableMap(
                sa.getProductGroups().stream()
                    .collect(Collectors.toMap(Function.identity(), productGroupMapper::map)));

        List<ProductGroupTask> productGroupTasks =
            productGroupByBasePg.values().stream()
                .map(pg -> pg.serviceAgreement(sa))
            ...
            .flatMap(
                list -> {
                    ...
                    if (list.stream().anyMatch(StreamTask::isFailed)) {
                        String ids =
                            list.stream()
                                .filter(StreamTask::isFailed)
                                .map(ProductGroupTask::getData)
                                .map(ProductGroup::getInternalId)
                                .collect(Collectors.joining(" "));
                        task.error(
                            ENTITY_SERVICE_AGREEMENT,
                            OP_PROCESS_PRODUCTS,
                            ...
                            sa.getInternalId(),
                            sa.getExternalId(),
                            "failed to setup Product Groups: %s for Service Agreement:" + " %s",
                            ids,
                            sa.getExternalId());
                        ...
                        task.info(
                            ...
                            sa.getExternalId(),
                            sa.getInternalId(),
                            "Product Groups setup for Service Agreement: %s",
                            sa.getExternalId());
                    }
                    return Mono.just(task);
                });
    ...
    private Mono<UpdatedServiceAgreementTask> processJobProfiles(
        UpdatedServiceAgreementTask streamTask) {
        ...
        if (isEmpty(sa.getSaUsers()) && isEmpty(sa.getSaAdmins())) {
            streamTask.warn(
                ENTITY_SERVICE_AGREEMENT,
                OP_PROCESS_JOB_PROFILES,
                RESULT_REJECTED,
                sa.getExternalId(),
                sa.getInternalId(),
                "No Job Profile Users defined for Service Agreement");
            return Mono.just(streamTask);
        }
                ...
                task -> {
                    if (!task.isFailed()) {
                        task.info(
                            ...
                            sa.getExternalId(),
                            sa.getInternalId(),
                            "Job Profiles updated for Service Agreement: %s",
                            sa.getExternalId());
                    }
                    return Mono.just(task);
                });
    ...
    private Mono<UpdatedServiceAgreementTask> retrieveUserInternalIds(
        UpdatedServiceAgreementTask task) {
        UpdatedServiceAgreement sa = task.getData();
        ...
        return Flux.fromIterable(users)
            .flatMap(
                jpu ->
                    accessGroupService
                        .getUserByExternalId(jpu.getUser().getExternalId(), true)
                        .onErrorResume(
                            ...
                            e -> {
                                ...
                                task.error(
                                    ...
                                    e,
                                    e.getMessage(),
                                    "failure retrieving user for external" + " id: %s",
                ...
                internalUsers -> {
                    Map<String, GetUser> usersByExternalId =
                        internalUsers.stream()
                            .collect(Collectors.toMap(GetUser::getExternalId, Function.identity()));
                    users.stream()
                        .forEach(
                            jp -> {
                                ...
                                if (internalUser != null) {
                                    ...
                                    task.error(
                                        ...
                                        sa.getExternalId(),
                                        sa.getInternalId(),
                                        "unable to find internal user for"
                                            + " external id: %s, this user will"
                                            + " be ignored",
                                        externalId);
                                }
                            });
    ...
    private Mono<UpdatedServiceAgreementTask> retrieveInternalProducts(
        UpdatedServiceAgreementTask task) {
        UpdatedServiceAgreement sa = task.getData();
        ...
        Map<String, BaseProduct> productByExternalId =
            unmodifiableMap(
                sa.getProductGroups().stream()
                    .flatMap(
                        pg ->
        ...
        return arrangementService
            .getArrangementByExternalId(new ArrayList<>(productByExternalId.keySet()))
            .doOnNext(
                arrangement -> {
                    log.debug(
                        "setting internal id {} for product with external id {}",
                        arrangement.getId(),
            ...
            .flatMap(
                list -> {
                    ...
                    if (task.isFailed()) {
                        return Mono.error(
                            new StreamTaskException(task, "error retrieving product internal id"));
                    }
                    return Mono.just(task);
                });
    }
                ...
                e -> {
                    ...
                    return Mono.error(
                        new StreamTaskException(
                            task,
                            MessageFormat.format(
                                "Failed to retrieve Function Groups for Service" + " Agreement: {0}",
                                sa.getExternalId())));
                })
            .map(
                list ->
                    list.stream().map(businessFunctionGroupMapper::map).collect(Collectors.toList()));
    }

    private Mono<UpdatedServiceAgreementTask> setupUserPermissions(
        UpdatedServiceAgreementTask task,
        ...
        BatchProductGroupTask bpgTask =
            new BatchProductGroupTask(
                ...
                new BatchProductGroup().serviceAgreement(sa),
                BatchProductIngestionMode.UPSERT);
        return accessGroupService
            .assignPermissionsBatch(bpgTask, request)
            .onErrorResume(
                e -> e instanceof StreamTaskException || e instanceof WebClientResponseException,
                e -> {
                    ...
                    bpgTask.error(
                        ...
                        e,
                        e.getMessage(),
                        "failed to assign permissions for service agreement: %s",
                        sa.getExternalId());
