...
class UpdatedServiceAgreementSagaTest {
    ...
    @InjectMocks private UpdatedServiceAgreementSaga updatedServiceAgreementSaga;

    @Mock private AccessGroupService accessGroupService;

    @Mock private ArrangementService arrangementService;

    @Captor private ArgumentCaptor<ProductGroupTask> productGroupTaskCaptor;
    ...
    void updateServiceAgreement() {
        ...
        UpdatedServiceAgreement serviceAgreement =
                new UpdatedServiceAgreement()
                        ...
                        .addSaAdminsItem(
                                new ServiceAgreementUserAction().userProfile(jobProfileUser1))
                        .addSaUsersItem(
                                new ServiceAgreementUserAction().userProfile(jobProfileUser2));
        serviceAgreement
        ...
        List<FunctionGroupItem> serviceAgreementFunctionGroups =
                asList(
                        new FunctionGroupItem()
                                .name("someJobRole1")
                                .type(FunctionGroupItem.TypeEnum.DEFAULT),
                        new FunctionGroupItem()
                                .name("someJobRole2")
                                .type(FunctionGroupItem.TypeEnum.DEFAULT),
                        new FunctionGroupItem()
        ...
        when(accessGroupService.updateServiceAgreementAssociations(
                        eq(task), eq(serviceAgreement), any()))
        ...
        when(accessGroupService.getUserByExternalId(eq("someUserExId1"), eq(true)))
                .thenReturn(
                        Mono.just(new GetUser().id("someUserInId1").externalId("someUserExId1")));
        when(accessGroupService.getUserByExternalId(eq("someUserExId2"), eq(true)))
                .thenReturn(
                        Mono.just(new GetUser().id("someUserInId2").externalId("someUserExId2")));
                ...
                .thenReturn(
                        Flux.fromIterable(
                                Collections.singletonList(
                                        new AccountArrangementItem()
                                                .id(loanInId)
                                                .externalArrangementId(loanExId))));
        ...
        verify(accessGroupService)
                .updateServiceAgreementAssociations(any(), eq(serviceAgreement), any());
        ...
        permissionUser1.put(
                new BusinessFunctionGroup().name("someJobRole1"), asList(baseProductGroup));
        Map<BusinessFunctionGroup, List<BaseProductGroup>> permissionUser2 = new HashMap<>();
        permissionUser2.put(
                new BusinessFunctionGroup().name("someJobRole2"), asList(baseProductGroup));
        Map<User, Map<BusinessFunctionGroup, List<BaseProductGroup>>> permissionsRequest =
                new HashMap<>();
