...
public class LegalEntityAsyncController implements AsyncApi {
    ...
    private final UpdatedServiceAgreementUnitOfWorkExecutor
            updatedServiceAgreementUnitOfWorkExecutor;
    private final UnitOfWorkMapper unitOfWorkMapper = Mappers.getMapper(UnitOfWorkMapper.class);

    @Override
    public Mono<ResponseEntity<LegalEntityResponse>> getUnitOfWork(
            String unitOfWorkId, ServerWebExchange exchange) {
        return legalEntityUnitOfWorkExecutor
                .retrieve(unitOfWorkId)
                .map(unitOfWorkMapper::convertToLegalEntityResponse)
                .map(ResponseEntity::ok);
    }

    @Override
    public Mono<ResponseEntity<UpdatedServiceAgreementResponse>>
            getUpdateServiceAgreementUnitOfWork(String unitOfWorkId, ServerWebExchange exchange) {
        return updatedServiceAgreementUnitOfWorkExecutor
                .retrieve(unitOfWorkId)
    ...
    @Override
    public Mono<ResponseEntity<Flux<LegalEntityResponse>>> processLegalEntitiesAsync(
            Flux<LegalEntity> legalEntity, ServerWebExchange exchange) {
        Flux<LegalEntityResponse> map =
                legalEntity
    ...
    public Mono<ResponseEntity<Flux<UpdatedServiceAgreementResponse>>> updateServiceAgreementAsync(
        ...
        Flux<UpdatedServiceAgreementResponse> map =
    ...
    private UnitOfWork<LegalEntityTask> createUnitOfWork(List<LegalEntity> legalEntities) {
        List<LegalEntityTask> tasks =
                legalEntities.stream().map(LegalEntityTask::new).collect(Collectors.toList());
    ...
    private UnitOfWork<UpdatedServiceAgreementTask> createServiceAgreementUnitOfWork(
        ...
        List<UpdatedServiceAgreementTask> tasks =
                serviceAgreements.stream()
