...
public class ContactsUnitOfWorkExecutor extends UnitOfWorkExecutor<ContactsTask> {
    ...
    public Flux<UnitOfWork<ContactsTask>> prepareUnitOfWork(List<ContactsBulkPostRequestBody> items) {
        ...
        items.forEach(item -> {
            ContactsTask contactsTask = new ContactsTask(unitOfWorkId + "-" + item.getAccessContext().getExternalUserId(), item);
            Flux<UnitOfWork<ContactsTask>> just = Flux.just(UnitOfWork.from(unitOfWorkId, contactsTask));
