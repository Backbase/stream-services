...
public class ContactsSaga implements StreamTaskExecutor<ContactsTask> {
  ...
  public Mono<ContactsTask> executeTask(ContactsTask contactsTask) {
    ...
    return contactsApi
        .postContactsBulk(item)
        .map(
            contactsBulkPostResponse -> {
              contactsTask.setResponse(contactsBulkPostResponse);
              contactsTask.info(
                  ENTITY,
                  CREATE,
                  SUCCESS,
                  item.getAccessContext().getExternalUserId(),
                  null,
            ...
            throwable -> {
              if (contactsWorkerConfigurationProperties.isContinueOnError()) {
                contactsTask.warn(
                    ...
                    item.getAccessContext().getExternalUserId(),
                    null,
                    "Failed to ingest contact but continueOnError is enabled "
                ...
                contactsTask.error(
                    ...
                    null,
                    throwable,
                    "Failed to ingest contact " + throwable.getMessage(),
                    FAILED_TO_INGEST_CONTACTS);
                return Mono.error(
                    new StreamTaskException(contactsTask, throwable, FAILED_TO_INGEST_CONTACTS));
              }
            });
  }
