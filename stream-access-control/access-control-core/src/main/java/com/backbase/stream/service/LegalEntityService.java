...
public class LegalEntityService {
  ...
  @NonNull private final LegalEntitiesApi legalEntitiesApi;
  @NonNull private final LegalEntityApi legalEntityApi;
  ...
  private final AccessGroupMapper serviceAgreementMapper =
      Mappers.getMapper(AccessGroupMapper.class);
  ...
  public Mono<LegalEntity> createLegalEntity(LegalEntity legalEntity) {
    ...
    return createLegalEntity(legalEntityCreateItem)
        .map(
            createdLegalEntityId -> {
              log.info(
                  "Created Legal Entity: {} with ID: {}",
                  legalEntity.getName(),
                  createdLegalEntityId.getId());
  ...
  private Mono<LegalEntityItemId> createLegalEntity(LegalEntityCreateItem legalEntity) {
    ...
    return legalEntitiesApi
        .postCreateLegalEntities(legalEntity)
        .doOnError(WebClientResponseException.class, this::handleWebClientResponseException)
        .onErrorResume(
            WebClientResponseException.class,
            exception ->
                Mono.error(
                    new LegalEntityException(
                        legalEntity, "Failed to create Legal Entity", exception)))
        .onErrorStop();
  }
   ...
   */
  public Mono<ServiceAgreement> getMasterServiceAgreementForExternalLegalEntityId(
      String legalEntityExternalId) {
    return legalEntitiesApi
        .getMasterServiceAgreementByExternalLegalEntity(legalEntityExternalId)
        .doOnNext(
            serviceAgreementItem ->
                log.info(
                    "Service Agreement: {} found for legal entity: {}",
        ...
        .onErrorResume(
            ...
            throwable -> {
              log.info(
                  "Master Service Agreement not found for: {}. Request:[{}] {} " + " Response: {}",
                  legalEntityExternalId,
   ...
   * @return Service Agreement
   */
  public Mono<ServiceAgreement> getMasterServiceAgreementForInternalLegalEntityId(
      String legalEntityInternalId) {
    log.info("Getting Service Agreement for: {}", legalEntityInternalId);
    return legalEntityApi
        .getMasterServiceAgreement(legalEntityInternalId)
        .doOnNext(
            serviceAgreementItem ->
                log.info(
                    "Service Agreement: {} found for legal entity: {}",
  ...
  public Mono<LegalEntity> getLegalEntityByExternalId(String externalId) {
    try {
      return legalEntitiesApi
          .getLegalEntityByExternalId(externalId)
          .onErrorResume(
              WebClientResponseException.NotFound.class,
              notFound -> {
                log.info(
                    "Legal Entity with externalId: {} does not exist: {}",
                    externalId,
  ...
  public Mono<LegalEntity> getLegalEntityByInternalId(String internalId) {
    return legalEntitiesApi
        .getLegalEntityById(internalId)
        .onErrorResume(
            WebClientResponseException.NotFound.class,
            notFound -> {
              log.info(
                  "Legal Entity with internalId: {} does not exist: {}",
                  internalId,
                  notFound.getResponseBodyAsString());
              return Mono.empty();
            })
  ...
  private void handleWebClientResponseException(
   ...
   *
   * <p>This implementation is brought here to avoid calling the
   *
   * @return access token.
   */
  private String getAccessControlAccessToken() {
    try {
      SecretKeySpec secretKeySpec =
          new SecretKeySpec(accessControlApiTokenKey.getBytes(StandardCharsets.UTF_8), "AES");
      IvParameterSpec iv =
          new IvParameterSpec(accessControlApiTokenInitPhrase.getBytes(StandardCharsets.UTF_8));
  ...
  public Mono<LegalEntity> putLegalEntity(LegalEntity legalEntity) {
    ...
    return legalEntitiesApi
        .putLegalEntities(Collections.singletonList(legalEntityPut))
        .doOnError(WebClientResponseException.class, this::handleWebClientResponseException)
        .onErrorResume(
            WebClientResponseException.class,
            exception ->
