...
public class BatchResponseUtils {
     ...
     */
    public <T> T checkBatchResponseItem(T response, String operation, String status, String resourceId,
        List<String> errors) {
        ...
        if (status != null && !HttpStatus.valueOf(Integer.parseInt(status)).is2xxSuccessful()) {
            String statusText = MessageFormat.format(
            ...
            if (validateAtomicResponse) {
                throw new WebClientResponseException(Integer.parseInt(status), statusText, null, null, null);
            } else {
