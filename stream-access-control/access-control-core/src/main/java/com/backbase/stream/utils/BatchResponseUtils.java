...
public class BatchResponseUtils {
    ...
    /**
     * Utility method to check Batch Response Item and throw WebClientResponseException if response
     * item status is not 200.
     *
        ...
        if (status != null && !HttpStatus.valueOf(Integer.parseInt(status)).is2xxSuccessful()) {
            String statusText =
                    MessageFormat.format(
                            "Failed item in the batch for {0}: status {1} for resource {2}, errors:"
                                    + " {3}",
