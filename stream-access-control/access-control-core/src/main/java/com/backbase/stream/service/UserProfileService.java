...
public class UserProfileService {
            ...
            .flatMap(
                getUserProfile ->
                    updateUserProfile(
                        ...
                        mapper.toUpdate(requestBody).id(getUserProfile.getId())))
            .doOnNext(
                existingUser ->
                    log.info(
                        "User Profile updated for User with ID: {}", existingUser.getExternalId()));
    Mono<GetUserProfile> createNewUser =
        createUserProfile(requestBody)
            .doOnNext(
                createdUser ->
                    log.info(
                        "User Profile created for User with ID: {}", createdUser.getExternalId()));
  ...
  public Mono<GetUserProfile> getUserProfileByUserID(String userId) {
    ...
    return userProfileApi
        ...
        .doOnNext(
            userProfileItem ->
                log.info("Found User Profile for externalId: {}", userProfileItem.getExternalId()))
        .onErrorResume(
