...
public class UserService {
  ...
  public Mono<User> getUserByExternalId(String externalId) {
    return usersApi
        .getUserByExternalId(externalId, true)
        .doOnNext(
            userItem ->
                log.info(
                    "Found user: {} for externalId: {}",
                    userItem.getFullName(),
                    userItem.getExternalId()))
        .onErrorResume(
            WebClientResponseException.NotFound.class,
            notFound -> handleUserNotFound(externalId, notFound.getResponseBodyAsString()))
        .map(mapper::toStream);
  }

  private Mono<? extends GetUser> handleUserNotFound(
      String externalId, String responseBodyAsString) {
   ...
   */
  public Mono<GetUsersList> getUsersByLegalEntity(
      String legalEntityInternalId, int size, int from) {
  ...
  /**
   * Archive users. As it is not possible to remove users from DBS, to be able to remove Legal
   * Entity user external ID is being updated to random value. (REMOVED_<external_id>_UUID) This
   * step is required to be able re-ingest user with same external id again.
   *
  ...
  public Mono<Void> archiveUsers(String legalEntityInternalId, List<String> userExternalIds) {
    //  There is no way to remove user from DBS, so to bypass this we just archive DBS user
    // representing member.
    return usersApi
        .updateUserInBatch(
                ...
                .map(
                    userExternalId -> {
                      return new BatchUser()
                          ...
                          .userUpdate(
                              new com.backbase.dbs.user.api.service.v2.model.User()
                                  .externalId(
                                      REMOVED_PREFIX + userExternalId + "_" + UUID.randomUUID())
        ...
        .map(
            r -> {
              log.debug(
                  "Batch Archive User response: status {} for resource {}, errors: {}",
                  r.getStatus(),
                  r.getResourceId(),
                  r.getErrors());
              if (r.getStatus().getValue() != null
                  && !HttpStatus.valueOf(Integer.parseInt(r.getStatus().getValue()))
                      .is2xxSuccessful()) {
  ...
  public Mono<List<User>> ingestUsers(List<User> users) {
    List<com.backbase.dbs.user.api.service.v2.model.User> userList =
        users.stream().map(mapper::toService).collect(Collectors.toList());
    return Mono.zip(
        Mono.just(users),
        usersApi
            .ingestUsers(userList)
            .onErrorContinue(
                WebClientResponseException.class,
                (throwable, o) -> {
            ...
            .collectList(),
        this::mergeBatchResults);
  }

  @NotNull
  private List<User> mergeBatchResults(List<User> users, List<BatchResponseItem> batchResponseItems)
      throws UserUpsertException {
    // current and batchResponseItems lists must be the same size;
    if (users.size() != batchResponseItems.size()) {
      throw new UserUpsertException(
          "Batch Results response does not match request", users, batchResponseItems);
    } else {
  ...
  private Mono<Realm> createRealm(final String realmName) {
    ...
    return identityManagementApi
        .createRealm(assignRealmRequest)
        .doOnNext(addRealmResponse -> log.info("Realm Created: '{}'", addRealmResponse.getId()))
        .onErrorResume(
            WebClientResponseException.class,
            e -> {
              log.error(
                  "Error creating realm: {} Response: {}", realmName, e.getResponseBodyAsString());
              return Mono.error(e);
            })
  ...
  private Mono<Realm> existingRealm(final String realmName) {
    ...
    return identityManagementApi
        ...
        .onErrorResume(
            ...
            e -> {
              log.error(
                  "Error getting realm: {} Response: {}", realmName, e.getResponseBodyAsString());
              return Mono.error(e);
            })
  ...
  public Mono<LegalEntity> linkLegalEntityToRealm(LegalEntity legalEntity) {
    ...
    log.info(
        "Linking Legal Entity with internal Id '{}' to Realm: '{}'",
    ...
    return identityManagementApi
        .assignRealm(legalEntity.getRealmName(), assignRealm)
        .onErrorResume(
            WebClientResponseException.class,
            e -> {
   ...
   */
  public Mono<User> createOrImportIdentityUser(
      User user, String legalEntityInternalId, StreamTask streamTask) {
    ...
    if (IdentityUserLinkStrategy.CREATE_IN_IDENTITY.equals(user.getIdentityLinkStrategy())) {
      Objects.requireNonNull(
          user.getFullName(),
          "User Full Name is required for user: "
              + user.getExternalId()
              + " in legal entity: "
              + legalEntityInternalId);
      Objects.requireNonNull(
          Optional.ofNullable(user.getEmailAddress()).map(EmailAddress::getAddress).orElse(null),
          "User Email Address is required for user: "
              + user.getExternalId()
              + " in legal entity: "
              + legalEntityInternalId);
      Objects.requireNonNull(
          Optional.ofNullable(user.getMobileNumber()).map(PhoneNumber::getNumber).orElse(null),
          "User Mobile Number is required for user: "
              + user.getExternalId()
              + " in legal entity: "
              + legalEntityInternalId);
      ...
      upsertCall =
          identityManagementApi
              ...
              .onErrorResume(
                  ...
                  e -> {
                    log.error(
                        "Error creating identity: {} Response: {}",
      ...
      upsertCall =
          identityManagementApi
              ...
              .onErrorResume(
                  ...
                  e -> {
                    log.error(
                        "Error importing identity: {} Response: {}",
                        importIdentity,
                        e.getResponseBodyAsString());
  ...
  private Mono<User> updateIdentityUser(User user, StreamTask streamTask) {
    if (userManagementProperties.isUpdateIdentity()
        ...
        && (!CollectionUtils.isEmpty(user.getAttributes())
            || !CollectionUtils.isEmpty(user.getAdditions()))) {
      ...
      return identityManagementApi
          .updateIdentity(user.getInternalId(), replaceIdentity)
          .onErrorResume(
              WebClientResponseException.class,
              e -> {
                log.error(
                    "Error updating identity: {} Response: {}",
                    user.getExternalId(),
                    e.getResponseBodyAsString());
                String message = "Failed to update identity: " + user.getExternalId();
                streamTask.error(
                    "user",
                    "update-identity-attributes-and-additions",
                    "failed",
                    user.getExternalId(),
                    user.getInternalId(),
                    e,
    ...
    log.debug(
        "The identity: {} update call is skipped as the specified update conditions are not met.",
        user.getExternalId());
  ...
  public Mono<User> updateUserState(User user, String realm) {
    log.info(
        "Changing user {} state to locked status {}, additional realm roles {}, additional groups {}",
        user.getInternalId(),
        user.getLocked(),
        user.getAdditionalRealmRoles(),
        user.getAdditionalGroups());

    return identityIntegrationApi
        .map(
            api ->
                getIdentityUser(user, realm)
                    .flatMap(
                        currentUser -> {
                          Optional<UserRequestBody> updateRequestBody =
                              updateRequired(currentUser, user);

                          if (updateRequestBody.isPresent()) {
                            return api.updateUserById(
                                realm, user.getInternalId(), updateRequestBody.get());
                          }
                    ...
                    .onErrorResume(
                        ...
                        e -> {
                          log.error(
                              "Error updated user {} state due to error: {}",
                              user.getInternalId(),
                              e.getResponseBodyAsString());
                          return Mono.error(e);
                        })
  ...
  private Mono<EnhancedUserRepresentation> getIdentityUser(User user, String realm) {
    return identityIntegrationApi
        .map(
            api ->
                api.getUserById(realm, user.getInternalId())
                    .doOnNext(
                        eur -> {
                    ...
                    .onErrorResume(
                        ...
                        e -> {
                          log.error(
                              "Error retrieving identity user {}: {}",
                              user.getInternalId(),
                              e.getResponseBodyAsString());
                          return Mono.error(e);
                        }))
  ...
  private Optional<UserRequestBody> updateRequired(
      EnhancedUserRepresentation currentUser, User user) {
    boolean updateRequired = false;
    UserRequestBody userRequestBody = mapper.toPresentation(currentUser);

    if (currentUser.getEnabled() != null) {
      boolean shouldEnable =
          user.getLocked() == null ? currentUser.getEnabled() : !user.getLocked();
  ...
  public Mono<User> updateIdentity(User user) {
    ...
    return identityManagementApi
        .getIdentity(user.getInternalId())
        .map(mapper::mapUpdateIdentity)
        .flatMap(
            updateIdentityRequest -> {
              log.debug(
                  "Trying to update identity attributes and additions, externalId [{}]",
                  user.getExternalId());
              ...
              updateIdentityRequest
                  .getAttributes()
                  .putAll(requireNonNullElse(user.getAttributes(), Map.of()));
              updateIdentityRequest
                  .getAdditions()
                  .putAll(requireNonNullElse(user.getAdditions(), Map.of()));

              if (userManagementProperties.isUpdateIdentity()) {
                return identityManagementApi
                    .updateIdentity(user.getInternalId(), updateIdentityRequest)
                    .onErrorResume(
                        WebClientResponseException.class,
                        e -> {
                          log.error(
                              "Failed to update identity: {}", e.getResponseBodyAsString(), e);
                          return Mono.error(e);
                        });
              }
              log.debug(
                  "The identity: {} update call is skipped because Update Identity flag is set to: {} ",
                  user.getExternalId(),
                  userManagementProperties.isUpdateIdentity());
  ...
  public Mono<User> updateUser(User user) {
    return usersApi
        ...
        .map(
            r -> {
              log.debug(
                  "Updated user response: status {} for resource {}, errors: {}",
                  r.getStatus(),
                  r.getResourceId(),
                  r.getErrors());
              if (r.getStatus().getValue() != null
                ...
                String errorMsg =
                    MessageFormat.format(
                        ...
                        r.getStatus(), r.getResourceId(), r.getErrors());
                throw new UserUpsertException(
                    errorMsg, Collections.singletonList(user), Collections.singletonList(r));
              }
              return r;
            })
        .collectList()
