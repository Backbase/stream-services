...
public class UserService {
              ...
              if (r.getStatus().getValue() != null
                ...
                throw new RuntimeException(
                    MessageFormat.format(
                        "Failed item in the batch for User Update: status"
                            + " {0} for resource {1}, errors: {2}",
  ...
  private Mono<Realm> existingRealm(final String realmName) {
    ...
    return identityManagementApi
        ...
        .map(
            realms ->
                realms.stream().filter(realm -> realmName.equals(realm.getRealmName())).findFirst())
  ...
  public Mono<Realm> setupRealm(LegalEntity legalEntity) {
    ...
    return existingRealm.switchIfEmpty(createNewRealm).map(actual -> actual);
  }
  ...
  public Mono<User> updateUserState(User user, String realm) {
    log.info(
        "Changing user {} state to locked status {}, additional realm roles {}, additional"
            + " groups {}",
        user.getInternalId(),
        user.getLocked(),
