...
public class UserService {
    ...
    public Mono<Void> archiveUsers(String legalEntityInternalId, List<String> userExternalIds) {
        ...
        return usersApi.updateUserInBatch(
                                ...
                                .map(
                                        userExternalId -> {
                                            return new BatchUser()
                                                    ...
                                                    .userUpdate(
                                                            new com.backbase.dbs.user.api.service.v2
                                                                            .model.User()
                                                                    .externalId(
                                                                            REMOVED_PREFIX
                                                                                    + userExternalId
                ...
                .map(
                        r -> {
                            log.debug(
                                    "Batch Archive User response: status {} for resource {},"
                                            + " errors: {}",
                                    r.getStatus(),
                                    r.getResourceId(),
                                    r.getErrors());
                            if (r.getStatus().getValue() != null
                                    && !HttpStatus.valueOf(
                                                    Integer.parseInt(r.getStatus().getValue()))
                                            .is2xxSuccessful()) {
    ...
    @NotNull
    private List<User> mergeBatchResults(
            List<User> users, List<BatchResponseItem> batchResponseItems)
        ...
        if (users.size() != batchResponseItems.size()) {
            ...
            for (int i = 0; i < users.size(); i++) {
                ...
                if (batchResponseItem.getErrors().isEmpty()) {
                    ...
                    throw new UserUpsertException(
                            "Failed to upsert user", users, batchResponseItems);
                }
            }
    ...
    private Mono<Realm> createRealm(final String realmName) {
        ...
        return identityManagementApi
                ...
                .doOnNext(
                        addRealmResponse ->
                                log.info("Realm Created: '{}'", addRealmResponse.getId()))
                .onErrorResume(
                        WebClientResponseException.class,
                        e -> {
                            log.error(
                                    "Error creating realm: {} Response: {}",
                                    realmName,
                                    e.getResponseBodyAsString());
                            return Mono.error(e);
                        })
                    ...
                    .onErrorResume(
                            ...
                            e -> {
                                log.error(
                                        ...
                                        e.getResponseBodyAsString());
                                String message =
                                        "Failed to update identity: " + user.getExternalId();
                                streamTask.error(
                                        "user",
                        ...
                        api ->
                                getIdentityUser(user, realm)
                                        ...
                                        .onErrorResume(
                                                ...
                                                e -> {
                                                    log.error(
                                                            "Error updated user {} state due to"
                                                                    + " error: {}",
                                                            user.getInternalId(),
                                                            e.getResponseBodyAsString());
                                                    return Mono.error(e);
