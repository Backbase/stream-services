...
import com.backbase.dbs.user.api.service.v2.model.GetIdentity;
...
class UserServiceTest {
  ...
  @Mock private UserProfileManagementApi userManagerProfileApi;
  ...
  void setup() {
    ...
    subject =
        new UserService(
            ...
            Optional.of(identityIntegrationApi),
  ...
  void updateUserStateAdditionalRealmRoles() {
    ...
    EnhancedUserRepresentation enhancedUserRepresentation =
        new EnhancedUserRepresentation().id(internalId).email(email).addRealmRolesItem("banking");
  ...
  void assignAdditionalGroups() {
    ...
    User user =
        new User().internalId(internalId).additionalGroups(Arrays.asList("group2", "group3"));
    Mono<User> result = subject.updateUserState(user, realm);
  ...
  void createOrImportIdentityUserUpdateAttributesWhenIFIStrategy() {
    ...
    User user =
        new User().externalId(externalId).attributes(attributesMap).identityLinkStrategy(strategy);
    ...
    ImportIdentity expectedImportIdentityRequest =
        new ImportIdentity().externalId(externalId).legalEntityInternalId(legalEntityId);
  ...
  void createOrImportIdentityUserUpdateAdditionsWhenIFIStrategy() {
    ...
    verify(identityManagementApi).importIdentity(expectedImportIdentityRequest);
    verify(identityManagementApi)
        .updateIdentity(internalId, new UpdateIdentityRequest().additions(additionsMap));
  }
  ...
  void createOrImportIdentityErrorOnImport() {
    ...
    when(identityManagementApi.importIdentity(any()))
        .thenReturn(
            Mono.error(
                WebClientResponseException.create(
                    500,
  ...
  void getUserByExternalId() {
    ...
    StepVerifier.create(userByExternalId).assertNext(Assertions::assertNotNull).verifyComplete();
  }
  ...
  void getUserProfile_notFound() {
    ...
    when(userManagerProfileApi.getUserProfile(userId))
        .thenReturn(
            Mono.error(
                WebClientResponseException.NotFound.create(
                    404, "not found", new HttpHeaders(), new byte[0], null)));
  ...
  void update_Identity() {
    final String userId = UUID.randomUUID().toString();
    userManagementProperties.setUpdateIdentity(true);
    when(identityManagementApi.getIdentity(anyString()))
        .thenReturn(Mono.just(new GetIdentity().externalId("").externalId(userId)));
    ...
    StepVerifier.create(result).expectNext(user).expectComplete().verify();
  }
  ...
  void update_Identity_Error() {
    ...
    when(identityManagementApi.getIdentity(anyString()))
        .thenReturn(Mono.just(new GetIdentity().externalId("").externalId(userId)));
    when(identityManagementApi.updateIdentity(anyString(), any()))
        .thenReturn(
            Mono.error(
                WebClientResponseException.create(
                    500,
