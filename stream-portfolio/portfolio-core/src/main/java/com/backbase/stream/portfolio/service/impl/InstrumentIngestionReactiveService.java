...
public class InstrumentIngestionReactiveService implements InstrumentIngestionService {
    ...
    public Flux<InstrumentBundle> ingestInstruments(Flux<InstrumentBundle> instrumentBundles) {
        return instrumentBundles
                .flatMap(instrumentIntegrationService::upsertInstrument, portfolioSagaProperties.getTaskExecutors())
                .doOnNext(actual -> log.info("Finished Ingestion of Instrument, name: {}",
                        Optional.ofNullable(actual.getInstrument()).map(Instrument::getName).orElse("null")));
    }
