...
public class PortfolioIntegrationService {
    ...
    public Mono<PositionBundle> upsertPosition(PositionBundle positionBundle) {
        ...
        return positionManagementApi.getPositionById(position.getExternalId())
            ...
            .flatMap(r -> positionManagementApi.putPosition(position.getExternalId(),
                    portfolioMapper.mapPutPosition(position))
    ...
    public Flux<PortfolioBundle> upsertPortfolio(PortfolioBundle portfolioBundle) {
        ...
        return portfolioManagementApi.getPortfolio(portfolio.getCode())
            ...
            .switchIfEmpty(portfolioManagementApi.postPortfolios(portfolioMapper.mapPortfolio(portfolio))
                ...
                .map(r -> portfolio))
            .thenMany(upsertSubPortfolio(portfolioBundle, portfolio))
            .thenMany(upsertAllocation(portfolioBundle, portfolio))
            ...
            .thenMany(upsertValuations(portfolioBundle, portfolio))
    ...
    private Mono<Void> upsertTransactions(PositionBundle positionBundle, String portfolioId, Position position) {
        return Mono.justOrEmpty(positionBundle.getTransactions())
            .flatMap(trs -> transactionManagementApi.deletePositionTransactions(position.getExternalId())
                .then(transactionManagementApi.postPortfolioTransactions(portfolioId,
    ...
    private Flux<Void> upsertTransactionCategory(PositionBundle positionBundle) {
        return transactionCategoryManagementApi.getTransactionCategories()
            ...
            .collectList()
            .flatMapMany(tcs ->
                ReactiveStreamHandler.getFluxStream(positionBundle.getTransactionCategories())
                    .flatMap(tc ->
    ...
    @NotNull
    private Flux<SubPortfolio> upsertSubPortfolio(PortfolioBundle portfolioBundle, Portfolio portfolio) {
        return ReactiveStreamHandler.getFluxStream(portfolioBundle.getSubPortfolios())
            .flatMap(sp -> subPortfolioManagementApi
                .getSubPortfolio(portfolio.getCode(), sp.getCode())
                .flatMap(r -> subPortfolioManagementApi
                    .putSubPortfolio(portfolio.getCode(), sp.getCode(),
    ...
    @NotNull
    private Mono<Void> upsertAllocation(PortfolioBundle portfolioBundle, Portfolio portfolio) {
    ...
    @NotNull
    private Mono<Void> upsertHierarchy(PortfolioBundle portfolioBundle,
        Portfolio portfolio) {
    ...
    @NotNull
    private Mono<Void> upsertPerformance(PortfolioBundle portfolioBundle,
        Portfolio portfolio) {
    ...
    private Mono<Void> upsertValuations(PortfolioBundle portfolioBundle, Portfolio portfolio) {
        return Flux.just(GranularityEnum.values())
            .flatMap(g -> portfolioValuationManagementApi.deletePortfolioValuations(portfolio.getCode(), g.getValue())
                .map(r -> g))
            .collectList()
            .then(Mono.justOrEmpty(portfolioBundle.getValuations())
