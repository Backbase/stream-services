...
public class InstrumentIntegrationService {
    ...
    public Mono<List<RegionBundle>> upsertRegions(List<RegionBundle> regionBundles) {
        return instrumentRegionManagementApi.getRegion(0, Integer.MAX_VALUE)
            .map(rr -> rr.getRegions().stream()
                .collect(Collectors.toMap(RegionsGetItem::getCode, RegionsGetItem::getName)))
            .switchIfEmpty(Mono.just(Collections.emptyMap()))
            .flatMap(regionNameByCode -> ReactiveStreamHandler.getFluxStream(regionBundles)
                .flatMap(rb -> {
                    ...
                    return upsertRegion(regionNameByCode, region)
                        ...
                        .onErrorResume(WebClientResponseException.class,
                            ReactiveStreamHandler.error(region, "Failed to upsert Region"))
                        .onErrorStop()
                        .map(o -> rb);
                })
                .collectList()
    ...
    public Mono<List<AssetClassBundle>> upsertAssetClass(List<AssetClassBundle> assetClassBundles) {
        return assetClassManagementApi.getAssetClasses(0, Integer.MAX_VALUE)
            .map(ac -> ac.getAssetClasses().stream()
                .collect(Collectors.toMap(AssetClassesGetItem::getCode, Function.identity())))
            .switchIfEmpty(Mono.just(Collections.emptyMap()))
            .flatMap(acByCode -> ReactiveStreamHandler.getFluxStream(assetClassBundles)
                .flatMap(ab -> {
    ...
    public Mono<InstrumentBundle> upsertInstrument(InstrumentBundle instrumentBundle) {
        ...
        return instrumentManagementApi.getInstrument(instrument.getId())
            .onErrorResume(WebClientResponseException.NotFound.class, throwable -> Mono.empty())
            .flatMap(i -> instrumentManagementApi
                .putInstrument(instrument.getId(), instrumentMapper.mapPutInstrument(instrument))
                .then(Mono.just(instrument))
            )
            .switchIfEmpty(instrumentManagementApi
    ...
    @NotNull
    private Mono<List<SubAssetClass>> upsertSubAssetClasses(AssetClassBundle assetClassBundle, AssetClass assetClass) {
        return assetClassManagementApi.getSubAssetClasses(assetClass.getCode(), 0, Integer.MAX_VALUE)
            .map(ac -> ac.getSubAssetClasses().stream()
                .collect(Collectors.toMap(SubAssetClassesGetItem::getCode, Function.identity())))
            .switchIfEmpty(Mono.just(Collections.emptyMap()))
            .flatMap(sacByCode -> ReactiveStreamHandler.getFluxStream(assetClassBundle.getSubAssetClasses())
                .flatMap(sac -> upsertSubAssetClassItem(assetClass.getCode(), sac, sacByCode)
                    .doOnError(WebClientResponseException.class,
                        ReactiveStreamHandler::handleWebClientResponseException)
                    .onErrorResume(WebClientResponseException.class,
                        ReactiveStreamHandler.error(assetClass, "Failed to create Asset Class"))
    ...
    private Mono<Void> upsertCountries(RegionBundle regionBundle, Region region) {
        return instrumentCountryManagementApi.getCountriesByRegion(region.getCode(), 0, Integer.MAX_VALUE)
            .map(cr -> cr.getCoutries().stream()
                .collect(Collectors.toMap(CountriesGetItem::getCode, CountriesGetItem::getName)))
            .switchIfEmpty(Mono.just(Collections.emptyMap()))
            .flatMap(countryNameByCode -> ReactiveStreamHandler.getFluxStream(regionBundle.getCountries())
                .flatMap(c -> upsertCountry(countryNameByCode, region, c)
                    .doOnError(WebClientResponseException.class,
                        ReactiveStreamHandler::handleWebClientResponseException)
                    .onErrorResume(WebClientResponseException.class,
    ...
    private Mono<Region> upsertRegion(Map<String, String> regionNameByCode, Region region) {
        if (regionNameByCode.containsKey(region.getCode())) {
            if (Objects.requireNonNullElse(region.getName(), "").equals(regionNameByCode.get(region.getCode()))) {
                return Mono.just(region);
            }
            log.debug("Update Region: {}", region);
            return instrumentRegionManagementApi.putRegion(region.getCode(), region.getName(), region.getCode())
                .map(r -> region);
        }
    ...
    private Mono<Country> upsertCountry(Map<String, String> countryNameByCode, Region region, Country country) {
        if (countryNameByCode.containsKey(country.getCode())) {
            if (Objects.requireNonNullElse(country.getName(), "").equals(countryNameByCode.get(country.getCode()))) {
                return Mono.just(country);
            }
            log.debug("Update Country: {}", country);
            return instrumentCountryManagementApi.putCountry(country.getCode(), country.getName(), country.getCode())
                .map(r -> country);
        }
        log.debug("Create Country: {}", country);
        return instrumentCountryManagementApi.postCountry(instrumentMapper.mapCountry(region.getCode(), country))
            .map(r -> country);
    }

    private Mono<AssetClass> upsertAssetClassItem(AssetClass assetClass, Map<String, AssetClassesGetItem> acByCode) {
        if (acByCode.containsKey(assetClass.getCode())) {
            ...
            return assetClassManagementApi.putAssetClass(assetClass.getCode(),
                    instrumentMapper.mapPutAssetClass(assetClass))
                .map(o -> assetClass);
        } else {
    ...
    private Mono<SubAssetClass> upsertSubAssetClassItem(String assetClassCode,
        SubAssetClass subAssetClass, Map<String, SubAssetClassesGetItem> acByCode) {
        if (acByCode.containsKey(subAssetClass.getCode())) {
            ...
            return assetClassManagementApi.putSubAssetClass(subAssetClass.getCode(),
                    instrumentMapper.mapPutSubAssetClass(subAssetClass))
                .map(o -> subAssetClass);
        } else {
            log.debug("Create SubAssetClass: {}", subAssetClass);
            return assetClassManagementApi.postSubAssetClass(assetClassCode,
                    instrumentMapper.mapSubAssetClass(subAssetClass))
                .map(o -> subAssetClass);
        }
    }
