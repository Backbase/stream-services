...
public class InstrumentIntegrationService {
                ...
                .map(
                        rr ->
                                rr.getRegions().stream()
                                        .collect(
                                                Collectors.toMap(
                                                        RegionsGetItem::getCode,
                                                        RegionsGetItem::getName)))
                                ...
                                ReactiveStreamHandler.getFluxStream(assetClassBundles)
                                        .flatMap(
                                                ab -> {
                                                    ...
                                                    return upsertAssetClassItem(
                                                                    assetClass, acByCode)
                                                            .thenMany(
                                                                    upsertSubAssetClasses(
                                                                            ab, assetClass))
                                                            .map(at -> assetClass)
                                                            .doOnError(
                                                                    WebClientResponseException
    ...
    public Mono<InstrumentBundle> upsertInstrument(InstrumentBundle instrumentBundle) {
        ...
        return instrumentManagementApi
                ...
                .flatMap(
                        i ->
                                instrumentManagementApi
                                        .putInstrument(
                                                instrument.getId(),
                                                instrumentMapper.mapPutInstrument(instrument))
                ...
                .onErrorResume(
                        WebClientResponseException.class,
                        ReactiveStreamHandler.error(
                                instrumentBundle, "Failed to create Instrument"))
                .onErrorStop();
    }
                ...
                .map(
                        ac ->
                                ac.getSubAssetClasses().stream()
                                        .collect(
                                                Collectors.toMap(
                                                        SubAssetClassesGetItem::getCode,
                                                        Function.identity())))
                .switchIfEmpty(Mono.just(Collections.emptyMap()))
                .flatMap(
                        sacByCode ->
                                ReactiveStreamHandler.getFluxStream(
                                                assetClassBundle.getSubAssetClasses())
                                        .flatMap(
                                                sac ->
                                                        upsertSubAssetClassItem(
                                                                        assetClass.getCode(),
                                                                        sac,
                                                                        sacByCode)
                                                                .doOnError(
    ...
    @NotNull
    private Mono<Void> updateHistoryPrices(
            InstrumentBundle instrumentBundle, Instrument instrument) {
                            ...
                            return instrumentPriceManagementApi
                                    .putInstrumentHistoryPrices(
                                            ...
                                            new InstrumentPricesHistoryPutRequest()
                                                    .priceData(
                                                            instrumentMapper.mapHistoryPrices(hps)))
                                    .doOnError(
                                            WebClientResponseException.class,
                                            ReactiveStreamHandler::handleWebClientResponseException)
                                    .onErrorResume(
                                            WebClientResponseException.class,
                                            ReactiveStreamHandler.error(
                                                    hps, "Failed to create History Prices"))
                                    .onErrorStop();
                        });
    }
                ...
                .map(
                        cr ->
                                cr.getCoutries().stream()
                                        .collect(
                                                Collectors.toMap(
                                                        CountriesGetItem::getCode,
                                                        CountriesGetItem::getName)))
                .switchIfEmpty(Mono.just(Collections.emptyMap()))
                .flatMap(
                        countryNameByCode ->
    ...
    private Mono<AssetClass> upsertAssetClassItem(
        ...
        if (acByCode.containsKey(assetClass.getCode())) {
            ...
            return assetClassManagementApi
                    .putAssetClass(
                            assetClass.getCode(), instrumentMapper.mapPutAssetClass(assetClass))
                    .map(o -> assetClass);
        } else {
    ...
    private Mono<SubAssetClass> upsertSubAssetClassItem(
        ...
        if (acByCode.containsKey(subAssetClass.getCode())) {
            ...
            return assetClassManagementApi
                    .postSubAssetClass(
                            assetClassCode, instrumentMapper.mapSubAssetClass(subAssetClass))
                    .map(o -> subAssetClass);
        }
    }
}
