...
class CompositeApiClientConfigTest {
    ...
    @Mock private Factory<ServiceInstance> loadBalancerFactory;
    ...
    void shouldReturnDirectUriWhenWithoutLoadBalancerAndWithDirectUriTest() {
        contextRunner
                .withPropertyValues(
                        "backbase.communication.services.user.profile.direct-uri=http://my-custom-uri/context")
                .withBean(WebClientAutoConfiguration.class)
                .withBean(InterServiceWebClientConfiguration.class)
    ...
    void shouldNotReturnDefaultServicePortWhenServicePortIsSetTest() {
        contextRunner
                .withPropertyValues(
                        "backbase.communication.http.default-service-port=8181",
                ...
                .run(
                        context -> {
                            var config = context.getBean(UserProfileManagerClientConfig.class);
                            assertEquals(
                                    "http://user-profile-manager:8080", config.createBasePath());
                        });
    }
        ...
        contextRunner
                ...
                .run(
                        context -> {
                            var config = context.getBean(UserProfileManagerClientConfig.class);
                            assertEquals(
                                    "http://user-profile-manager:8181", config.createBasePath());
                        });
    }
}
