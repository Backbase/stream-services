...
public class InvestmentAssetUniversSaga implements StreamTaskExecutor<InvestmentAssetsTask> {
    ...
    public Mono<InvestmentAssetsTask> createAssetCategories(InvestmentAssetsTask investmentTask) {
        InvestmentAssetData investmentData = investmentTask.getData();
        int categoryCount = investmentData.getAssetCategories() != null ? investmentData.getAssetCategories().size() : 0;
        log.info("Starting investment asset category creation: taskId={}, categoryCount={}",
                investmentTask.getId(), categoryCount);

        investmentTask.info(INVESTMENT, OP_CREATE, null, investmentTask.getName(), investmentTask.getId(),
                PROCESSING_PREFIX + categoryCount + " investment asset categories");
        investmentTask.setState(State.IN_PROGRESS);

        if (categoryCount == 0) {
            log.warn("No asset categories to create for taskId={}", investmentTask.getId());
            investmentTask.setState(State.COMPLETED);
            return Mono.just(investmentTask);
        }
        ...
        return Flux.fromIterable(investmentData.getAssetCategories())
                ...
                .map(assetCategories -> {
                    ...
                    investmentTask.setState(State.COMPLETED);
                    log.info("Successfully created all asset categories: taskId={}, categoryCount={}",
                            investmentTask.getId(), assetCategories.size());
                    return investmentTask;
                })
                .doOnError(throwable -> {
                    log.error("Failed to create investment asset categories: taskId={}, categoryCount={}",
                            investmentTask.getId(), categoryCount, throwable);
                    investmentTask.error(INVESTMENT, OP_CREATE, RESULT_FAILED, investmentTask.getName(),
                            investmentTask.getId(),
                            "Failed to create investment asset categories: " + throwable.getMessage());
                });
    }

    public Mono<InvestmentAssetsTask> createAssetCategoryTypes(InvestmentAssetsTask investmentTask) {
        InvestmentAssetData investmentData = investmentTask.getData();
        int typeCount = investmentData.getAssetCategoryTypes() != null ? investmentData.getAssetCategoryTypes().size() : 0;
        log.info("Starting investment asset category type creation: taskId={}, typeCount={}",
                investmentTask.getId(), typeCount);

        investmentTask.info(INVESTMENT, OP_CREATE, null, investmentTask.getName(), investmentTask.getId(),
                PROCESSING_PREFIX + typeCount + " investment asset category types");
        investmentTask.setState(State.IN_PROGRESS);

        if (typeCount == 0) {
            log.warn("No asset category types to create for taskId={}", investmentTask.getId());
            investmentTask.setState(State.COMPLETED);
            return Mono.just(investmentTask);
        }

        return Flux.fromIterable(investmentData.getAssetCategoryTypes())
                .flatMap(assetCategoryType -> {
                ...
                .map(assetCategoryTypes -> {
                    ...
                    investmentTask.info(INVESTMENT, OP_CREATE, RESULT_CREATED, investmentTask.getName(),
                            investmentTask.getId(),
                            RESULT_CREATED + " " + assetCategoryTypes.size() + " Investment Asset Category Types");
                    investmentTask.setState(State.COMPLETED);
                    log.info("Successfully created all asset category types: taskId={}, typeCount={}",
                            investmentTask.getId(), assetCategoryTypes.size());
                    return investmentTask;
                })
                .doOnError(throwable -> {
                    log.error("Failed to create investment asset category types: taskId={}, typeCount={}",
                            investmentTask.getId(), typeCount, throwable);
                    investmentTask.error(INVESTMENT, OP_CREATE, RESULT_FAILED, investmentTask.getName(),
                            investmentTask.getId(),
                            "Failed to create investment asset category types: " + throwable.getMessage());
                });
    }
