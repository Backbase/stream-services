    ...
    public Mono<ClientUser> upsertClient(@NotNull ClientCreateRequest request) {
        ...
        return clientApi.listClients(List.of(), null,
            ...
            .flatMap(clients -> {
                ...
                log.debug("Patching client uuid={} with payload={}", client.getUuid(), patch);
                return clientApi.patchClient(client.getUuid(), patch)
                    .doOnError(throwable -> log.error("Patch client failed", throwable))
                    .map(updated -> {
                        log.info("Patched existing investment client uuid={} internalUserId={}",
                            client.getUuid(), client.getInternalUserId());
                        return ClientUser.builder()
                            .investmentUserId(client.getUuid())
                            .externalUserId((String) client.getExtraData().get("user_external_id"))
                            .internalUserId(client.getInternalUserId())
                            .build();
                    });*/

                return Mono.just(ClientUser.builder()
                    .investmentUserId(client.getUuid())
                    .externalUserId((String) client.getExtraData().get("user_external_id"))
                    .internalUserId(client.getInternalUserId())
                    .build()
